...
//  TicTacToe
//
//  Created by sadiyah khanam on 01/01/2024.
//
...
class ViewController: UIViewController {
    ...
    func initBoard(){
        // add all buttons to a board array for tracking
        board.append(a1)
        board.append(a2)
        board.append(a3)
        board.append(b1)
        ...
        for button in board{
            button.titleLabel?.font = .systemFont(ofSize: 60.0, weight: .heavy)
    ...
    func fullBoard() -> Bool{
    // if any button if untouched then game is not over
        for button in board{
